...
// Identite XGS, commune au portail et aux autres outils (Horizon, Meridien,
// Mon Salaire) : le bleu nuit du logo pour l'encre, la barre laterale et les
// boutons principaux ; le jaune soleil pour l'accent de marque (bouton
// d'action, pastilles, reperes). L'ambre reste une couleur d'ETAT (a rappeler,
// pause) : le soleil n'est pas lisible en texte sur fond blanc.
const C = {
  ink: "#000B53", inkSoft: "#0B1766", inkFaint: "#1B2878", inkDeep: "#00052E", canvas: "#F4F5FA",
  surface: "#FFFFFF", border: "#E4E6F0", borderSoft: "#ECEEF5", text: "#10133A",
  muted: "#5F6282", mutedSoft: "#8A8FAE", amber: "#E0932B", amberSoft: "#FBF0DD",
  sun: "#FDCF4F", sunSoft: "#FFF4D1", sunHover: "#FFD968",
  lavande: "#C7CBEB", lavandeDouce: "#A9AED6",
  teal: "#1B7A8C", tealSoft: "#E1F0F2", green: "#2E9B6B", greenSoft: "#E3F5EC",
  red: "#C6493F", redSoft: "#FBEAE8",
};
const FONTS = "https://fonts.googleapis.com/css2?family=Poppins:wght@500;600;700&family=Inter:wght@400;500;600;700&family=IBM+Plex+Mono:wght@400;500&display=swap";
...
const STYLES_INTERACTION = `
  :where(button, a, input, select, textarea, [tabindex]):focus-visible {
    outline: 2px solid ${C.ink} !important; outline-offset: 2px;
  }
  aside :where(button, a, input, select, textarea, [tabindex]):focus-visible { outline-color: ${C.sun} !important; }
  main h1.disp::after { content: "."; color: ${C.sun}; }
  ...
  .bouton-sortie:hover { border-color: ${C.sun} !important; color: #FFFFFF !important; filter: none; }
  .nav-item:not(.nav-actif):hover { background: rgba(255,255,255,.06) !important; color: #FFFFFF !important; filter: none; }
  ...
  .zone-saisie:focus-within { border-color: ${C.inkSoft} !important; box-shadow: 0 0 0 3px rgba(11,23,102,.10); }
  .zone-saisie textarea:focus-visible { outline: none !important; }
  @keyframes pouls { 0% { box-shadow: 0 0 0 0 rgba(46,155,107,.45); } 70% { box-shadow: 0 0 0 6px rgba(46,155,107,0); } 100% { box-shadow: 0 0 0 0 rgba(46,155,107,0); } }
  .point-direct { animation: pouls 2s ease-out infinite; }
...
// Soleil XGS en filigrane, repris du portail : decor des fonds bleu nuit.
function SoleilFiligrane({ taille = 320, style }) {
  const rayons = Array.from({ length: 12 }, (_, i) => i * 30);
  return (
    <svg aria-hidden="true" width={taille} height={taille} viewBox="0 0 200 200" style={{ position: "absolute", pointerEvents: "none", ...style }}>
      <g fill="none" stroke={C.sun} strokeLinecap="round">
...
// "Kouadio Aya Marie" -> "KA" : pastille d'une personne, dans la barre
// laterale, la messagerie et la supervision.
function initiales(nom) {
  const mots = String(nom || "").replace(/@.*/, "").split(/[\s._-]+/).filter(Boolean);
  return (mots.slice(0, 2).map((m) => m[0]).join("") || "?").toUpperCase();
}

// Couleur de pastille stable par personne : le meme collegue garde la meme
// teinte d'un ecran a l'autre. Teintes sombres, lisibles avec du blanc.
const TEINTES_PERSONNE = ["#0B1766", "#1B7A8C", "#2E7D5B", "#7A4E9E", "#A8412F", "#8A5A08", "#335C99", "#5F6282"];
function teintePersonne(id) {
  let h = 0;
  for (const ch of String(id || "")) h = (h * 31 + ch.charCodeAt(0)) >>> 0;
  return TEINTES_PERSONNE[h % TEINTES_PERSONNE.length];
}
  ...
  if (restoring) {
    return (
      <div className="flex items-center justify-center" style={{ minHeight: "100vh", background: C.ink }}>
        <div className="flex items-center gap-2" style={{ color: "#A9AED6", fontSize: 13 }}>
...
function LoginScreen({ onLogin, loading, error }) {
  ...
  return (
    <div style={{
      ...
      <style>{`@import url('${FONTS}'); .disp{font-family:'Poppins',sans-serif;} * { box-sizing: border-box; } button{cursor:pointer;font-family:inherit;} input{font-family:inherit;} @keyframes spin { to { transform: rotate(360deg); } } ${STYLES_INTERACTION}`}</style>
      ...
      <div style={{ position: "relative", display: "flex", flexDirection: "column", alignItems: "center", gap: 26, zIndex: 1 }}>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
          ...
          <div style={{ textAlign: "center" }}>
            ...
            <p style={{ fontSize: 12.5, color: "#8F94BC", marginTop: 3 }}>Connexion à votre espace</p>
          </div>
        </div>
        ...
        >
          <div style={{ marginBottom: 16 }}>
            <label style={{ fontSize: 11, fontWeight: 600, color: "#8F94BC", letterSpacing: "0.02em", textTransform: "uppercase" }}>Login</label>
            <input
          ...
          <div style={{ marginBottom: 22 }}>
            <label style={{ fontSize: 11, fontWeight: 600, color: "#8F94BC", letterSpacing: "0.02em", textTransform: "uppercase" }}>Mot de passe</label>
            <input
      ...
      <div style={{ position: "relative", display: "flex", flexDirection: "column", alignItems: "center", gap: 24, zIndex: 1 }}>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
          ...
          <div style={{ textAlign: "center" }}>
            ...
            <p style={{ fontSize: 12.5, color: "#8F94BC", marginTop: 4, maxWidth: 320, lineHeight: 1.5 }}>
        ...
        <form onSubmit={handleSubmit} style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", backdropFilter: "blur(20px)", borderRadius: 18, padding: 28, width: 340 }}>
          <label style={{ fontSize: 11, fontWeight: 600, color: "#8F94BC", textTransform: "uppercase" }}>Nouveau mot de passe</label>
          ...
          <label style={{ fontSize: 11, fontWeight: 600, color: "#8F94BC", textTransform: "uppercase" }}>Confirmer le mot de passe</label>
        ...
        <button onClick={onLogout} style={{ background: "none", border: "none", color: "#8F94BC", fontSize: 12 }}>
          Se déconnecter
        </button>
      </div>
    </div>
...
function Workspace({ session, onLogout, onProfilChange }) {
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif", background: C.canvas, color: C.text, minHeight: "100vh" }}>
      <style>{`@import url('${FONTS}');
        .disp { font-family: 'Poppins', sans-serif; } .mono { font-family: 'IBM Plex Mono', monospace; }
      ...
      <div className="flex" style={{ minHeight: "100vh" }}>
        <aside style={{ width: 232, background: `linear-gradient(180deg, ${C.ink} 0%, ${C.inkDeep} 100%)`, color: "#fff", flexShrink: 0, position: "relative", overflow: "hidden" }} className="flex flex-col justify-between">
          <SoleilFiligrane taille={300} style={{ bottom: -90, left: -110, opacity: 0.07 }} />
          <div style={{ position: "relative" }}>
            <div className="px-5 pt-6 pb-5" style={{ borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
              <div className="flex items-center gap-2.5">
                <LogoMark size={30} />
                <div>
                  <div className="disp" style={{ fontSize: 19, fontWeight: 600, lineHeight: 1.1 }}>Auréo<span style={{ color: C.sun }}>.</span></div>
                  <div style={{ fontSize: 10.5, color: "#8F94BC", marginTop: 2 }}>Centre d'appels · XGS</div>
                </div>
              </div>
            </div>
            ...
            {isAdmin && (
              <div className="px-5 pt-4">
                <div style={{ background: "rgba(255,255,255,0.06)", borderRadius: 10, padding: 4 }} className="flex gap-1">
                  {["agent", "admin"].map((r) => (
                    <button key={r} onClick={() => { setRole(r); setAdminTab(r === "agent" ? "poste" : (ROLE_DEFAULT_TABS[profil?.role] || ["dashboard"])[0]); }}
                      style={{ flex: 1, padding: "7px 0", borderRadius: 7, border: "none", background: role === r ? C.sun : "transparent", color: role === r ? C.ink : "#A9AED6", fontSize: 12.5, fontWeight: 600 }}>
                      {r === "agent" ? "Agent" : "Admin"}
                    </button>
                  ))}
                </div>
          ...
          <div className="px-5 py-4" style={{ borderTop: "1px solid rgba(255,255,255,0.08)", position: "relative" }}>
            <div className="flex items-center gap-2.5 mb-2.5">
              <div className="disp flex items-center justify-center" style={{ width: 32, height: 32, borderRadius: 10, background: C.sun, color: C.ink, fontSize: 12, fontWeight: 600, flexShrink: 0 }}>
                {initiales(profil?.nom || session.user.email)}
              </div>
            ...
            <div style={{ position: "relative" }}>
              <div style={{ background: "rgba(255,255,255,0.06)", borderRadius: 9, padding: 4 }} className="flex gap-1 mb-3">
                {AGENT_STATUTS.map((st) => {
                  ...
                  return (
                    <button key={st.id} disabled={statutBusy} title={st.label}
                      ...
                      <Icon size={14} color={active ? C.ink : "#8F94BC"} />
                    </button>
                  );
                })}
              </div>
            ...
            <div style={{ fontSize: 10, color: "#8F94BC", textAlign: "center", marginBottom: 10 }}>
...
function NavItem({ icon: Icon, label, active, onClick, pastille }) {
  return (
    <button onClick={onClick} className={active ? "nav-item nav-actif" : "nav-item"} aria-current={active ? "page" : undefined}
      style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 12px", borderRadius: 9, border: "none", background: active ? "rgba(255,255,255,0.09)" : "transparent", boxShadow: active ? `inset 3px 0 0 ${C.sun}` : "none", color: active ? "#fff" : "#A9AED6", fontSize: 13, fontWeight: active ? 600 : 500, textAlign: "left" }}>
      <Icon size={16} strokeWidth={2} color={active ? C.sun : "currentColor"} />
      <span style={{ flex: 1 }}>{label}</span>
      {pastille > 0 && (
        <span className="mono" style={{ background: C.sun, color: C.ink, borderRadius: 999, padding: "1px 7px", fontSize: 10.5, fontWeight: 700 }}>
          ...
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, overflow: "hidden" }}>
            <div style={{ background: C.ink, color: "#fff", padding: "14px 18px" }}>
              <div className="flex items-center justify-between">
                <span className="mono flex items-center gap-3" style={{ fontSize: 11, letterSpacing: "0.04em", color: "#A9AED6" }}>
                ...
                <span style={{ fontSize: 10.5, background: C.sun, color: C.ink, padding: "2px 8px", borderRadius: 999, fontWeight: 600 }}>EN COURS</span>
              </div>
              {campagneInfo?.nom && (
    ...
    <div style={{ background: C.surface, border: `1.5px solid ${C.ink}`, borderRadius: 14, padding: 20, marginBottom: 20 }}>
      ...
      <div className="flex items-center gap-2 mt-4">
        <button onClick={handleCreate} disabled={busy}
          style={{ background: C.sun, color: C.ink, border: "none", borderRadius: 8, padding: "10px 18px", fontSize: 13, fontWeight: 700, display: "flex", alignItems: "center", gap: 6 }}>
    ...
    <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: "56px 24px", textAlign: "center" }}>
      ...
      <div className="flex items-center justify-center gap-3">
        <button onClick={onPull} disabled={pulling || !enProduction}
          style={{ background: C.sun, color: C.ink, border: "none", borderRadius: 9, padding: "11px 22px", fontSize: 13.5, fontWeight: 700, display: "inline-flex", alignItems: "center", gap: 8, opacity: enProduction ? 1 : 0.45, cursor: enProduction ? "pointer" : "not-allowed" }}>
...
function SupervisionPanel({ accessToken, callerRole }) {
  ...
  // Deux niveaux de filtre : l'equipe et la recherche choisissent le
  // perimetre (et donc les indicateurs) ; le filtre d'etat ne fait que
  // restreindre les cartes affichees, sans changer les chiffres du haut.
  ...
  }, [lignes, equipe, recherche]);

  const filtrees = useMemo(() => selection.filter((a) =>
    etat === "tous" ? true
      : etat === "alertes" ? a.alertes.length > 0
      : a.statut === etat), [selection, etat]);
  ...
  const trierMur = (a, b) => (ORDRE_STATUT_SUPERVISION[a.statut] ?? 3) - (ORDRE_STATUT_SUPERVISION[b.statut] ?? 3) || a.nom.localeCompare(b.nom);
  const actifs = filtrees.filter((a) => a.statut !== "deconnecte");
  const enAlerte = actifs.filter((a) => a.alertes.length > 0).sort(trierMur);
  const sansAlerte = actifs.filter((a) => a.alertes.length === 0).sort(trierMur);
  ...
  const ETATS = [
    ...
    { id: "en_prod", l: "Production", n: kpi.prod, couleur: C.green },
    { id: "en_pause", l: "Pause", n: kpi.pause, couleur: C.amber },
    { id: "deconnecte", l: "Déconnectés", n: kpi.deco, couleur: C.mutedSoft },
    { id: "alertes", l: "Alertes", n: kpi.alertes, couleur: C.red },
  ];

  const titreSection = (texte, n, couleur) => (
    <div className="flex items-center gap-2" style={{ fontSize: 11, color: couleur || C.muted, textTransform: "uppercase", letterSpacing: "0.05em", fontWeight: 700, margin: "4px 0 10px" }}>
      {texte} <span className="mono" style={{ fontWeight: 600, color: C.mutedSoft }}>{n}</span>
  ...
  return (
    <div>
      <div className="flex items-start justify-between mb-5" style={{ gap: 16, flexWrap: "wrap" }}>
        <div>
        ...
        <div className="flex items-center gap-2" style={{ flexShrink: 0 }}>
          <span className="flex items-center gap-2" title="Actualisé toutes les 30 secondes"
            style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 999, padding: "6px 12px", fontSize: 11.5, color: C.muted }}>
            <span className={error ? undefined : "point-direct"} style={{ width: 8, height: 8, borderRadius: 999, background: error ? C.red : C.green, flexShrink: 0 }} />
            {error ? "Connexion interrompue" : ilYa === null ? "Chargement…" : <>En direct · <span className="mono">{ilYa < 5 ? "à l'instant" : `il y a ${ilYa} s`}</span></>}
          </span>
          <button onClick={() => setSonActif((v) => !v)} title={sonActif ? "Couper le son des alertes" : "Activer le son des alertes"}
            className="flex items-center gap-2"
            style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 999, padding: "6px 12px", fontSize: 11.5, fontWeight: 600, color: sonActif ? C.text : C.mutedSoft }}>
            {sonActif ? <Volume2 size={14} color={C.ink} /> : <VolumeX size={14} color={C.mutedSoft} />}
            {sonActif ? "Son des alertes" : "Son coupé"}
          </button>
        </div>
      </div>
      ...
      {!agents ? (
        ...
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(165px, 1fr))", gap: 10, marginBottom: 12 }}>
            {/* Presence : la carte de reference, sur le bleu nuit XGS, avec la
                repartition production / pause / deconnectes. */}
            <div style={{ gridColumn: "span 2", position: "relative", overflow: "hidden", background: `linear-gradient(135deg, ${C.ink} 0%, ${C.inkSoft} 100%)`, color: "#fff", borderRadius: 14, padding: "14px 16px" }}>
              <SoleilFiligrane taille={170} style={{ top: -60, right: -50, opacity: 0.12 }} />
              <div style={{ position: "relative" }}>
                <div style={{ fontSize: 10.5, color: C.sun, textTransform: "uppercase", letterSpacing: "0.08em", fontWeight: 700 }}>Présents maintenant</div>
                <div className="flex items-baseline gap-2" style={{ marginTop: 4 }}>
                  <span className="disp" style={{ fontSize: 30, fontWeight: 600, lineHeight: 1 }}>{presents}</span>
                  <span style={{ fontSize: 12.5, color: C.lavande }}>sur {effectif} agent{effectif > 1 ? "s" : ""}</span>
                </div>
                <div className="flex" style={{ height: 6, borderRadius: 999, overflow: "hidden", background: "rgba(255,255,255,0.12)", marginTop: 12 }}>
                  {effectif > 0 && <>
                    <span style={{ width: `${(kpi.prod / effectif) * 100}%`, background: C.green }} />
                    <span style={{ width: `${(kpi.pause / effectif) * 100}%`, background: C.amber }} />
                  </>}
                </div>
                <div className="flex items-center" style={{ gap: 14, marginTop: 8, fontSize: 11.5, color: C.lavande, flexWrap: "wrap" }}>
                  <span className="flex items-center gap-1.5"><span style={{ width: 7, height: 7, borderRadius: 999, background: C.green }} />{kpi.prod} en production</span>
                  <span className="flex items-center gap-1.5"><span style={{ width: 7, height: 7, borderRadius: 999, background: C.amber }} />{kpi.pause} en pause</span>
                  <span className="flex items-center gap-1.5"><span style={{ width: 7, height: 7, borderRadius: 999, background: "rgba(255,255,255,0.3)" }} />{kpi.deco} déconnecté{kpi.deco > 1 ? "s" : ""}</span>
                </div>
              </div>
            </div>
            {[
              { titre: "Fiches traitées", valeur: kpi.fiches.toLocaleString("fr-FR"), detail: `${parHeure(kpi.fiches, kpi.secProd)} par heure de production`, icone: ListChecks },
              { titre: "Taux de contact", valeur: pct(kpi.contacts, kpi.fiches), detail: `${kpi.contacts.toLocaleString("fr-FR")} contacts`, icone: PhoneCall },
              ...
              <div key={c.titre} style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: "13px 15px" }}>
                <div className="flex items-center justify-between" style={{ gap: 6 }}>
                  <span style={{ fontSize: 10.5, color: C.muted, textTransform: "uppercase", letterSpacing: "0.04em", fontWeight: 600 }}>{c.titre}</span>
            ...
            <button onClick={() => setEtat((e) => (e === "alertes" ? "tous" : "alertes"))} aria-pressed={etat === "alertes"}
              title={kpi.alertes > 0 ? "Afficher seulement les agents en alerte" : undefined}
              style={{ textAlign: "left", background: kpi.alertes > 0 ? C.redSoft : C.surface, border: `1px solid ${kpi.alertes > 0 ? C.red : C.border}`, borderRadius: 14, padding: "13px 15px", boxShadow: etat === "alertes" ? `0 0 0 3px ${C.redSoft}` : "none" }}>
              <div className="flex items-center justify-between" style={{ gap: 6 }}>
                <span style={{ fontSize: 10.5, color: kpi.alertes > 0 ? C.red : C.muted, textTransform: "uppercase", letterSpacing: "0.04em", fontWeight: 600 }}>Alertes</span>
                <AlertTriangle size={13} color={kpi.alertes > 0 ? C.red : C.mutedSoft} />
              </div>
              <div className="disp" style={{ fontSize: 23, fontWeight: 600, marginTop: 5, color: kpi.alertes > 0 ? C.red : C.text }}>{kpi.alertes}</div>
              <div style={{ fontSize: 11, color: C.muted, marginTop: 2 }}>
                {kpi.alertes === 0 ? "rien à signaler" : etat === "alertes" ? "cliquer pour tout revoir" : `agent${kpi.alertes > 1 ? "s" : ""} à surveiller`}
              </div>
            </button>
          </div>

          {/* ---- seuils ---- */}
          <div className="flex items-center gap-2 mb-4" style={{ fontSize: 11.5, color: C.muted, flexWrap: "wrap" }}>
            <AlertTriangle size={12} color={C.mutedSoft} />
            <span>
              Alertes : même fiche au-delà de <strong style={{ color: C.text }}>{seuils.appel_long_minutes} min</strong> · en production sans fiche au-delà de <strong style={{ color: C.text }}>{seuils.inactivite_minutes} min</strong> · pauses selon les limites de leur type.
            </span>
            {peutRegler && !reglageOuvert && (
              <button onClick={() => { setBrouillonSeuils(seuils); setReglageOuvert(true); }}
                style={{ background: "none", border: "none", color: C.ink, fontSize: 11.5, fontWeight: 600, padding: 0, textDecoration: "underline", textDecorationColor: C.sun, textDecorationThickness: 2, textUnderlineOffset: 3 }}>
                Régler
              </button>
            )}
          </div>
          {reglageOuvert && (
            <div className="flex items-end gap-3 mb-4 apparait-bas" style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: "12px 14px", flexWrap: "wrap" }}>
              {[
                ...
                <label key={f.cle} style={{ fontSize: 11, color: C.muted, display: "flex", flexDirection: "column", gap: 4 }}>
                  ...
                  <input type="number" min={1} max={240} value={brouillonSeuils[f.cle]}
                    ...
                    style={{ width: 120, border: `1px solid ${C.border}`, borderRadius: 8, padding: "7px 9px", fontSize: 13 }} />
                </label>
              ))}
              <button onClick={enregistrerSeuils} disabled={enregistrement}
                style={{ display: "flex", alignItems: "center", gap: 6, background: C.ink, color: "#fff", border: "none", borderRadius: 9, padding: "9px 14px", fontSize: 12.5, fontWeight: 600 }}>
                {enregistrement ? <Loader2 size={13} className="animate-spin" /> : <Check size={13} />} Enregistrer
              </button>
              <button onClick={() => setReglageOuvert(false)}
                style={{ background: "none", border: `1px solid ${C.border}`, borderRadius: 9, padding: "9px 14px", fontSize: 12.5 }}>
                Annuler
              </button>
          ...
          <div className="flex items-center gap-2 mb-4" style={{ flexWrap: "wrap" }}>
            <div role="group" aria-label="Affichage" style={{ display: "flex", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: 3 }}>
              {[{ id: "mur", l: "Mur" }, { id: "classement", l: "Classement" }].map((v) => (
                <button key={v.id} onClick={() => setVue(v.id)} aria-pressed={vue === v.id}
                  style={{ background: vue === v.id ? C.ink : "transparent", color: vue === v.id ? "#fff" : C.text, border: "none", borderRadius: 7, padding: "6px 13px", fontSize: 12, fontWeight: 600 }}>
                  {v.l}
                </button>
              ))}
            </div>
            <select value={equipe} onChange={(e) => setEquipe(e.target.value)} aria-label="Équipe"
              style={{ border: `1px solid ${C.border}`, borderRadius: 10, padding: "8px 10px", fontSize: 12.5, background: C.surface, color: C.text }}>
              <option value="">Toutes les équipes</option>
              {equipes.map((e) => <option key={e} value={e}>{e}</option>)}
            </select>
            <div className="flex items-center gap-2" style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: "7px 10px" }}>
              <Search size={13} color={C.mutedSoft} />
              <input value={recherche} onChange={(e) => setRecherche(e.target.value)} placeholder="Nom ou matricule" aria-label="Rechercher un agent"
                style={{ border: "none", outline: "none", fontSize: 12.5, width: 150, background: "transparent" }} />
            </div>
            <div role="group" aria-label="État" className="flex items-center" style={{ gap: 5, flexWrap: "wrap" }}>
              {ETATS.map((e) => {
                const actif = etat === e.id;
                return (
                  <button key={e.id} onClick={() => setEtat(e.id)} aria-pressed={actif}
                    className="flex items-center gap-1.5"
                    style={{ background: actif ? C.ink : C.surface, color: actif ? "#fff" : C.text, border: `1px solid ${actif ? C.ink : C.border}`, borderRadius: 999, padding: "5px 11px", fontSize: 12, fontWeight: 600 }}>
                    {e.couleur && <span style={{ width: 7, height: 7, borderRadius: 999, background: e.couleur }} />}
                    {e.l}
                    <span className="mono" style={{ fontSize: 11, fontWeight: 500, color: actif ? C.lavande : C.mutedSoft }}>{e.n}</span>
                  </button>
                );
              })}
            </div>
          ...
          {filtrees.length === 0 ? (
            <EtatVide icon={Users}
              titre={agents.length === 0 ? "Aucun agent dans votre périmètre." : etat === "alertes" ? "Aucune alerte en cours." : "Aucun agent ne correspond à ces critères."}
              detail={agents.length === 0 ? "Les agents apparaissent ici dès qu'ils sont rattachés à vos équipes." : etat === "alertes" ? "Tout le monde est dans les limites. Revenez sur « Tous » pour voir le plateau." : "Changez d'équipe, videz la recherche ou revenez sur « Tous »."} />
          ) : vue === "mur" ? (
            <>
              {enAlerte.length > 0 && (
                <div style={{ marginBottom: 16 }}>
              ...
              {deconnectes.length > 0 && (
                <div style={{ marginTop: 4 }}>
                  {titreSection("Déconnectés", deconnectes.length)}
                  <div className="flex" style={{ flexWrap: "wrap", gap: 6 }}>
                    {deconnectes.map((a) => (
                      <span key={a.agent_id} className="flex items-center gap-1.5" title={a.fiches_jour > 0 ? `${a.fiches_jour} fiches aujourd'hui` : "Aucune fiche aujourd'hui"}
                        style={{ background: a.alertes.length > 0 ? C.redSoft : C.surface, border: `1px solid ${a.alertes.length > 0 ? C.red : C.border}`, borderRadius: 999, padding: "3px 10px 3px 3px", fontSize: 11.5, color: C.muted }}>
                        <span className="flex items-center justify-center" style={{ width: 20, height: 20, borderRadius: 999, background: C.canvas, color: C.muted, fontSize: 9, fontWeight: 700 }}>{initiales(a.nom)}</span>
                        {a.nom}{a.fiches_jour > 0 ? <span className="mono" style={{ color: C.mutedSoft }}>· {a.fiches_jour}</span> : ""}
                      </span>
                    ))}
                  </div>
                </div>
...
// Voile d'une couleur "#RRGGBB" : fond doux d'une pastille d'etat. Les couleurs
// de pause viennent de la base ; une valeur inattendue retombe sur le canevas.
function voileCouleur(hex, alpha = 0.14) {
  const m = /^#([0-9a-f]{6})$/i.exec(hex || "");
  if (!m) return C.canvas;
...
function CarteSupervision({ a, typesPause, maintenant, seuils, controle }) {
  ...
  return (
    <div style={{ background: C.surface, border: `1px solid ${enAlerte ? C.red : C.border}`, borderRadius: 14, padding: 14, boxShadow: `inset 3px 0 0 ${couleurStatut}${enAlerte ? `, 0 0 0 3px ${C.redSoft}` : ""}` }}>
      <div className="flex items-center" style={{ gap: 10 }}>
        <span className="disp flex items-center justify-center" aria-hidden="true"
          style={{ width: 36, height: 36, borderRadius: 11, background: teintePersonne(a.agent_id), color: "#fff", fontSize: 12.5, fontWeight: 600, flexShrink: 0 }}>
          {initiales(a.nom)}
        </span>
      ...
      <div style={{ marginTop: 11, background: C.canvas, borderRadius: 9, padding: "8px 10px", fontSize: 11.5 }}>
        {a.fiche_id ? (
      ...
      <div className="flex" style={{ marginTop: 11 }}>
        {[
          ...
          <div key={s.l} style={{ flex: 1, textAlign: "center", borderLeft: i > 0 ? `1px solid ${C.borderSoft}` : "none" }}>
            <div className="disp" style={{ fontSize: 14.5, fontWeight: 600 }}>{s.v}</div>
            <div style={{ fontSize: 10, color: C.mutedSoft, textTransform: "uppercase", letterSpacing: "0.03em" }}>{s.l}</div>
          </div>
        ))}
      </div>

      {enAlerte && (
        <div className="flex flex-col" style={{ marginTop: 11, gap: 4 }}>
          {a.alertes.map((al) => (
            <div key={al.code} className="flex items-center gap-1.5" style={{ background: C.redSoft, color: C.red, borderRadius: 7, padding: "5px 8px", fontSize: 11, fontWeight: 600 }}>
...
function ClassementSupervision({ lignes, tri, setTri, pct, parHeure }) {
  ...
  // Le podium n'a de sens que sur un classement du meilleur au moins bon.
  const podium = !col.texte && tri.sens < 0;
  const MEDAILLES = [C.sun, "#D5D9E6", "#E8B98A"];
  const couleurStatut = (a) => (a.statut === "en_prod" ? C.green : a.statut === "en_pause" ? C.amber : C.mutedSoft);

  return (
    <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, overflow: "auto", maxHeight: "calc(100vh - 260px)" }}>
      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12.5 }}>
        <thead>
          <tr style={{ textAlign: "left" }}>
            <th style={{ position: "sticky", top: 0, zIndex: 1, background: C.canvas, padding: "10px 12px", color: C.muted, fontWeight: 600, fontSize: 11 }}>#</th>
            {colonnes.map((c) => {
              const actif = tri.col === c.id;
              return (
                <th key={c.id} aria-sort={actif ? (tri.sens < 0 ? "descending" : "ascending") : undefined}
                  style={{ position: "sticky", top: 0, zIndex: 1, background: C.canvas, padding: 0, textAlign: c.texte ? "left" : "right", whiteSpace: "nowrap" }}>
                  <button onClick={() => setTri((t) => ({ col: c.id, sens: t.col === c.id ? -t.sens : (c.texte ? 1 : -1) }))}
                    style={{ width: "100%", background: "none", border: "none", padding: "10px 12px", color: actif ? C.ink : C.muted, fontWeight: actif ? 700 : 600, fontSize: 11, textTransform: "uppercase", letterSpacing: "0.03em", textAlign: c.texte ? "left" : "right" }}>
                    {c.l}{actif ? (tri.sens < 0 ? " ↓" : " ↑") : ""}
                  </button>
                </th>
              );
            })}
        ...
        <tbody>
          {triees.map((a, i) => {
            const alerte = a.alertes.length > 0;
            return (
              <tr key={a.agent_id} title={alerte ? a.alertes.map((al) => al.texte).join(" · ") : undefined}
                style={{ borderTop: `1px solid ${C.borderSoft}`, boxShadow: alerte ? `inset 3px 0 0 ${C.red}` : "none" }}>
                <td style={{ padding: "8px 12px" }}>
                  {podium && i < 3 ? (
                    <span className="mono flex items-center justify-center" style={{ width: 22, height: 22, borderRadius: 999, background: MEDAILLES[i], color: C.ink, fontSize: 11, fontWeight: 700 }}>{i + 1}</span>
                  ) : (
                    <span className="mono" style={{ color: C.mutedSoft, paddingLeft: 6 }}>{i + 1}</span>
                  )}
                </td>
                {colonnes.map((c) => (
                  <td key={c.id} className={c.texte ? undefined : "mono"}
                    style={{ padding: "8px 12px", textAlign: c.texte ? "left" : "right", fontWeight: c.id === "nom" || c.id === tri.col ? 600 : 400, color: c.id === tri.col ? C.ink : undefined, whiteSpace: "nowrap" }}>
                    {c.id === "nom" ? (
                      <span className="flex items-center gap-2">
                        <span style={{ width: 7, height: 7, borderRadius: 999, background: couleurStatut(a), flexShrink: 0 }} />
                        {a.nom}
                        {alerte && <AlertTriangle size={12} color={C.red} style={{ flexShrink: 0 }} />}
                      </span>
                ...
                <tr key={r.id} style={{ borderTop: `1px solid ${C.borderSoft}`, background: aTraiter ? C.amberSoft : decisionNonVue ? C.tealSoft : undefined, opacity: r.statut === "annulee" && !decisionNonVue ? 0.55 : 1 }}>
                  ...
                  <td style={{ padding: "9px 14px", textAlign: "right" }}>
                    <button onClick={() => setOuverte(r.id)}
                      style={{ background: aTraiter ? C.sun : decisionNonVue ? C.teal : C.ink, color: aTraiter ? C.ink : "#fff", border: "none", borderRadius: 7, padding: "6px 12px", fontSize: 11.5, fontWeight: 700 }}>
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "300px 1fr", gap: 20, alignItems: "start" }}>
        ...
        <div className="flex flex-col gap-3">
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            <div style={{ background: `linear-gradient(135deg, ${C.ink} 0%, ${C.inkSoft} 100%)`, borderRadius: 14, padding: 20 }}>
              <div className="flex items-center gap-2" style={{ marginBottom: 10 }}>
                <CheckCircle2 size={14} color="#A9AED6" />
                <span style={{ fontSize: 12, color: "#A9AED6", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.03em" }}>Appels traités</span>
              </div>
              <div className="disp mono" style={{ fontSize: 34, fontWeight: 700, color: "#fff" }}>{stats ? stats.appels_traites : "…"}</div>
              <div style={{ fontSize: 11.5, color: "#8F94BC", marginTop: 4 }}>Sur les dates sélectionnées</div>
            </div>
            <div style={{ background: `linear-gradient(135deg, ${C.ink} 0%, ${C.inkSoft} 100%)`, borderRadius: 14, padding: 20 }}>
              <div className="flex items-center gap-2" style={{ marginBottom: 10 }}>
                <Award size={14} color={C.sun} />
                <span style={{ fontSize: 12, color: "#A9AED6", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.03em" }}>Rechargements validés</span>
              </div>
              <div className="disp mono" style={{ fontSize: 34, fontWeight: 700, color: C.sun }}>{stats ? stats.rechargements_valides : "…"}</div>
              <div style={{ fontSize: 11.5, color: "#8F94BC", marginTop: 4 }}>
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14, marginBottom: 24 }}>
        <div style={{ background: `linear-gradient(135deg, ${C.ink} 0%, ${C.inkSoft} 100%)`, borderRadius: 14, padding: 20 }}>
          <div className="flex items-center gap-2" style={{ marginBottom: 10 }}>
            <PhoneOff size={14} color="#A9AED6" style={{ transform: "rotate(135deg)" }} />
            <span style={{ fontSize: 12, color: "#A9AED6", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.03em" }}>Taux de joignabilité</span>
          </div>
        ...
        <div style={{ background: `linear-gradient(135deg, ${C.ink} 0%, ${C.inkSoft} 100%)`, borderRadius: 14, padding: 20 }}>
          <div className="flex items-center gap-2" style={{ marginBottom: 10 }}>
            <CheckCircle2 size={14} color={C.sun} />
            <span style={{ fontSize: 12, color: "#A9AED6", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.03em" }}>Taux de conversion</span>
          </div>
          <div className="disp mono" style={{ fontSize: 34, fontWeight: 700, color: C.sun }}>
        ...
        <div style={{ background: `linear-gradient(135deg, ${C.ink} 0%, ${C.inkSoft} 100%)`, borderRadius: 14, padding: 20 }}>
          <div className="flex items-center gap-2" style={{ marginBottom: 10 }}>
            <Clock size={14} color="#7FD1DE" />
            <span style={{ fontSize: 12, color: "#A9AED6", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.03em" }}>Temps moyen / fiche</span>
          </div>
          <div className="disp mono" style={{ fontSize: 34, fontWeight: 700, color: "#7FD1DE" }}>
      ...
      {rows === null ? (
        ...
        <>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginBottom: 24 }}>
            ...
            <div style={{ background: `linear-gradient(135deg, ${C.ink} 0%, ${C.inkSoft} 100%)`, borderRadius: 14, padding: 18 }}>
              ...
              <div style={{ fontSize: 12, color: "#8F94BC", marginTop: 2 }}>Taux d'occupation moyen</div>
            </div>
          </div>
    ...
    <div>
      ...
      <div className="flex items-center gap-3 mb-4" style={{ maxWidth: 760 }}>
        <button onClick={loadPreview} disabled={loading}
          style={{ display: "flex", alignItems: "center", gap: 7, background: C.sun, color: C.ink, border: "none", borderRadius: 8, padding: "10px 18px", fontSize: 13, fontWeight: 700, flexShrink: 0 }}>
          {loading ? <Loader2 size={14} className="animate-spin" /> : <Search size={14} />} Rechercher
        </button>
...
function Messagerie({ accessToken, moi, moiId, onLu, isSuperAdmin, sonActif, setSonActif, nonLusParCanal = {} }) {
  ...
  // Pastille d'une conversation : l'icone du canal, ou les initiales du
  // collegue dans sa teinte (la meme partout dans l'application).
  const Pastille = ({ type, id, nom, taille = 32 }) => (
    <span className="disp flex items-center justify-center" aria-hidden="true"
      style={{ width: taille, height: taille, borderRadius: Math.round(taille * 0.32), flexShrink: 0, color: "#fff", fontSize: Math.round(taille * 0.36), fontWeight: 600,
        background: type === "general" ? C.ink : type === "equipe" ? C.inkSoft : teintePersonne(id) }}>
      {type === "general" ? <Megaphone size={Math.round(taille * 0.45)} color={C.sun} />
        : type === "equipe" ? <UsersRound size={Math.round(taille * 0.45)} color={C.sun} />
        : initiales(nom)}
    </span>
  );

  const Entree = ({ actif, onClick, type, id, libelle, detail, cle }) => {
    const nonLus = (cle && nonLusParCanal[cle]) || 0;
    return (
      <button onClick={onClick} className="flex items-center gap-2.5 entree-canal" aria-current={actif ? "true" : undefined}
        style={{ width: "100%", textAlign: "left", background: actif ? C.canvas : "transparent", boxShadow: actif ? `inset 3px 0 0 ${C.sun}` : "none", border: "none", borderRadius: 10, padding: "7px 10px" }}>
        <Pastille type={type} id={id} nom={libelle} />
        <span style={{ minWidth: 0, flex: 1 }}>
          <span style={{ display: "block", fontSize: 12.5, fontWeight: nonLus > 0 || actif ? 600 : 500, color: C.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{libelle}</span>
          {detail && <span style={{ display: "block", fontSize: 10.5, color: C.mutedSoft, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{detail}</span>}
        </span>
        {nonLus > 0 && (
          <span className="mono" style={{ flexShrink: 0, background: C.sun, color: C.ink, borderRadius: 999, padding: "1px 7px", fontSize: 10.5, fontWeight: 700 }}>
            {nonLus > 99 ? "99+" : nonLus}
          </span>
        )}
      </button>
  ...
  const titreGroupe = (t) => (
    <div style={{ fontSize: 10, color: C.mutedSoft, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", padding: "10px 10px 4px" }}>{t}</div>
  );

  // Separateur de jour dans le fil : "Aujourd'hui", "Hier", puis la date.
  const libelleJour = (d) => {
    ...
    <div>
        ...
        <div className="flex items-center gap-1.5" style={{ flexShrink: 0 }}>
          ...
          {sonActif && (
            <button onClick={jouerCarillon} title="Écouter le son"
              style={{ background: "none", border: "none", padding: 6, fontSize: 11.5, color: C.muted, textDecoration: "underline", textUnderlineOffset: 3 }}>
              Écouter
            </button>
          )}
        </div>
      ...
      {isSuperAdmin && coupeEnProd !== null && (
        <div className="flex items-center gap-3 mb-4" style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: "12px 16px", maxWidth: 620 }}>
          <button onClick={basculerReglage} disabled={bascule} role="switch" aria-checked={coupeEnProd}
            title={coupeEnProd ? "Rendre la messagerie accessible en Production" : "Couper la messagerie aux agents en Production"}
            style={{ position: "relative", width: 38, height: 21, borderRadius: 999, border: "none", padding: 0, flexShrink: 0, background: coupeEnProd ? C.ink : C.border, transition: "background 0.15s" }}>
            <span style={{ position: "absolute", top: 2, left: coupeEnProd ? 19 : 2, width: 17, height: 17, borderRadius: 999, background: coupeEnProd ? C.sun : "#fff", transition: "left 0.15s" }} />
          </button>
          <div style={{ minWidth: 0, flex: 1 }}>
            <div style={{ fontSize: 12.5, fontWeight: 600 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "280px 1fr", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, overflow: "hidden", height: "max(540px, calc(100vh - 230px))" }}>
        {/* ---- conversations ---- */}
        <div style={{ borderRight: `1px solid ${C.borderSoft}`, display: "flex", flexDirection: "column", minHeight: 0 }}>
          <div style={{ padding: 10, borderBottom: `1px solid ${C.borderSoft}` }}>
            <div className="flex items-center gap-2" style={{ background: C.canvas, borderRadius: 10, padding: "8px 10px" }}>
              <Search size={13} color={C.mutedSoft} style={{ flexShrink: 0 }} />
              <input value={recherche} onChange={(ev) => setRecherche(ev.target.value)} placeholder="Écrire à un collègue…" aria-label="Chercher un collègue par son nom"
                style={{ flex: 1, minWidth: 0, border: "none", outline: "none", background: "transparent", fontSize: 12.5 }} />
              {recherche && (
                <button onClick={() => setRecherche("")} title="Effacer" style={{ background: "none", border: "none", padding: 0, display: "flex" }}>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "4px 8px 10px" }}>
            {recherche.trim().length >= 2 ? (
              <>
                {titreGroupe("Résultats")}
                {cherche ? (
                  <p className="flex items-center gap-2" style={{ fontSize: 11.5, color: C.mutedSoft, padding: "6px 10px" }}><Loader2 size={12} className="animate-spin" /> Recherche…</p>
                ) : resultats.length === 0 ? (
                  ...
                  resultats.map((p) => (
                    <Entree key={p.id} actif={canal.type === "direct" && canal.id === p.id} onClick={() => { setCanal({ type: "direct", id: p.id }); setRecherche(""); }}
                      type="direct" id={p.id} libelle={p.nom} detail={`${ROLE_COURT[p.role] || p.role}${p.equipe_nom ? " · " + p.equipe_nom : ""}`} cle={`direct:${p.id}`} />
                  ))
                )}
              </>
            ) : (
              <>
                {recherche.trim().length === 1 && <p style={{ fontSize: 11.5, color: C.mutedSoft, padding: "8px 10px 0" }}>Encore une lettre…</p>}
                {titreGroupe("Canaux")}
                <Entree actif={canal.type === "general"} onClick={() => setCanal({ type: "general" })} type="general" libelle="Général" detail="Tout le plateau" cle="general" />
                {equipes.map((e) => (
                  <Entree key={e.id} actif={canal.type === "equipe" && canal.id === e.id} onClick={() => setCanal({ type: "equipe", id: e.id })}
                    type="equipe" id={e.id} libelle={e.nom} detail={e.est_la_mienne ? "Mon équipe" : "Équipe"} cle={`equipe:${e.id}`} />
                ))}

                {titreGroupe("Messages directs")}
                {conversations.length === 0 ? (
                  <EtatVide compact icon={MessageSquare} titre="Aucune conversation." detail="Cherchez un collègue par son nom, en haut, pour lui écrire." />
                ) : (
                  conversations.map((p) => (
        ...
        <div style={{ display: "flex", flexDirection: "column", minHeight: 0, minWidth: 0 }}>
          <div className="flex items-center gap-3" style={{ padding: "12px 18px", borderBottom: `1px solid ${C.borderSoft}` }}>
            <Pastille type={canal.type} id={canal.id} nom={titreCanal} taille={36} />
            <div style={{ minWidth: 0 }}>
              <div className="disp" style={{ fontSize: 15, fontWeight: 600, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{titreCanal}</div>
              <div style={{ fontSize: 11, color: C.mutedSoft }}>
                {canal.type === "general" ? "Tout le plateau" : canal.type === "equipe" ? "L'équipe, son coach et l'encadrement" : "Conversation privée"}
              </div>
            </div>
          </div>

          <div ref={filRef} style={{ flex: 1, overflowY: "auto", padding: "14px 18px", background: "#F8F9FC" }}>
            {messages === null ? (
              <CenterLoader />
            ) : messages.length === 0 ? (
              ...
              <div className="flex flex-col">
                {messages.map((m, i) => {
                  ...
                  // Un groupe : meme auteur, meme jour, moins de 5 minutes d'ecart.
                  const memeAuteur = !nouveauJour && prec.auteur_id === m.auteur_id
                    && new Date(m.created_at) - new Date(prec.created_at) < 5 * 60000;
                  const suiv = messages[i + 1];
                  const finGroupe = !suiv || suiv.auteur_id !== m.auteur_id
                    || new Date(suiv.created_at).toDateString() !== new Date(m.created_at).toDateString()
                    || new Date(suiv.created_at) - new Date(m.created_at) >= 5 * 60000;
                  return (
                    <Fragment key={m.id}>
                      {nouveauJour && (
                        <div className="flex items-center gap-3" style={{ margin: i === 0 ? "0 0 12px" : "16px 0 12px" }}>
                          <span style={{ flex: 1, height: 1, background: C.border }} />
                          <span style={{ fontSize: 11, fontWeight: 600, color: C.muted, textTransform: "capitalize" }}>{libelleJour(m.created_at)}</span>
                          <span style={{ flex: 1, height: 1, background: C.border }} />
                        </div>
                      )}
                      <div id={`msg-${m.id}`} className="msg-ligne" style={{ display: "flex", gap: 8, justifyContent: deMoi ? "flex-end" : "flex-start", marginTop: memeAuteur ? 3 : 12, borderRadius: 10 }}>
                        {!deMoi && (
                          <span style={{ width: 30, flexShrink: 0 }}>
                            {!memeAuteur && <Pastille type="direct" id={m.auteur_id} nom={nomDe(m.auteur_id)} taille={30} />}
                          </span>
                        )}
                        <div style={{ maxWidth: "72%", minWidth: 0 }}>
                          {!memeAuteur && (
                            <div style={{ fontSize: 11, marginBottom: 3, textAlign: deMoi ? "right" : "left" }}>
                              {!deMoi && <span style={{ fontWeight: 600, color: C.text }}>{nomDe(m.auteur_id)} </span>}
                              <span style={{ color: C.mutedSoft }}>{heure(m.created_at)}</span>
                            </div>
                          )}
                          {m.repond_a && (
                            <button onClick={() => {
                                const cible = document.getElementById(`msg-${m.repond_a}`);
                                if (cible) { cible.scrollIntoView({ block: "center" }); cible.style.transition = "background 0.4s"; cible.style.background = C.sunSoft; setTimeout(() => { cible.style.background = "transparent"; }, 900); }
                              }}
                              style={{ display: "block", width: "100%", textAlign: deMoi ? "right" : "left", background: "none", border: "none", padding: 0, marginBottom: 3 }}>
                              <span style={{ display: "inline-block", maxWidth: "100%", borderLeft: `2px solid ${C.sun}`, background: C.surface, borderRadius: "0 8px 8px 0", padding: "3px 8px", fontSize: 11, color: C.mutedSoft, textAlign: "left" }}>
                          ...
                          <div className="flex items-center gap-1.5" style={{ flexDirection: deMoi ? "row-reverse" : "row" }}>
                            <div title={new Date(m.created_at).toLocaleString("fr-FR", { weekday: "long", day: "numeric", month: "long", hour: "2-digit", minute: "2-digit" })}
                              style={{ background: deMoi ? C.ink : C.surface, color: deMoi ? "#fff" : C.text, border: deMoi ? "none" : `1px solid ${C.border}`, borderRadius: deMoi ? `14px 14px ${finGroupe ? 4 : 14}px 14px` : `14px 14px 14px ${finGroupe ? 4 : 14}px`, padding: "8px 12px", fontSize: 13, lineHeight: 1.45, whiteSpace: "pre-wrap", wordBreak: "break-word", minWidth: 0 }}>
                              {m.contenu}
                            </div>
                            <div className={`flex items-center msg-actions${reactionOuverte === m.id ? " ouvert" : ""}`} style={{ position: "relative", flexShrink: 0, background: C.surface, border: `1px solid ${C.border}`, borderRadius: 999, padding: "1px 3px" }}>
                              <button onClick={() => { setRepondA(m); saisieRef.current?.focus(); }} title="Répondre à ce message" aria-label="Répondre"
                                style={{ background: "none", border: "none", padding: 3, display: "flex" }}>
                                <Reply size={13} color={C.muted} />
                              </button>
                              <button onClick={() => setReactionOuverte(reactionOuverte === m.id ? null : m.id)} title="Réagir" aria-label="Réagir"
                                style={{ background: "none", border: "none", padding: 3, display: "flex" }}>
                                <Smile size={13} color={reactionOuverte === m.id ? C.ink : C.muted} />
                              </button>
                              {deMoi && (
                                <button onClick={() => supprimer(m.id)} title="Supprimer mon message" aria-label="Supprimer"
                                  style={{ background: "none", border: "none", padding: 3, display: "flex" }}>
                                  <Trash2 size={12} color={C.muted} />
                                </button>
                              )}
                              {reactionOuverte === m.id && (
                                <div className="flex apparait-haut" style={{ position: "absolute", bottom: "calc(100% + 6px)", [deMoi ? "right" : "left"]: 0, background: C.surface, border: `1px solid ${C.border}`, borderRadius: 999, padding: "3px 5px", boxShadow: "0 8px 22px rgba(0,11,83,0.14)", zIndex: 15, gap: 1 }}>
                                  {REACTIONS_RAPIDES.map((e) => (
                                    <button key={e} onClick={() => basculerReaction(m, e)} title={e} className="emoji-choix"
                                      style={{ background: "none", border: "none", borderRadius: 999, padding: "2px 4px", fontSize: 16, lineHeight: 1.2 }}>
                                      {e}
                                    </button>
                                  ))}
                                </div>
                          ...
                          {(() => {
                            ...
                            return (
                              <div className="flex flex-wrap gap-1" style={{ marginTop: 4, justifyContent: deMoi ? "flex-end" : "flex-start" }}>
                                {[...parEmoji].map(([emoji, gens]) => {
                                  ...
                                  return (
                                    <button key={emoji} onClick={() => basculerReaction(m, emoji)}
                                      ...
                                      style={{ display: "flex", alignItems: "center", gap: 3, background: moiDedans ? C.sunSoft : C.surface, border: `1px solid ${moiDedans ? C.sun : C.border}`, borderRadius: 999, padding: "1px 8px", fontSize: 12, lineHeight: 1.5 }}>
                                      <span>{emoji}</span>
          ...
          {repondA && (
            <div className="flex items-center gap-2 apparait-haut" style={{ padding: "8px 14px 0" }}>
              <div className="flex items-center gap-2" style={{ flex: 1, minWidth: 0, background: C.canvas, borderLeft: `2px solid ${C.sun}`, borderRadius: "0 8px 8px 0", padding: "6px 10px" }}>
                <Reply size={12} color={C.ink} style={{ flexShrink: 0 }} />
                <span style={{ minWidth: 0, fontSize: 11.5, color: C.muted, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  Réponse à <strong style={{ color: C.text }}>{repondA.auteur_id === moiId ? "moi" : nomDe(repondA.auteur_id)}</strong>
                  {" · "}{repondA.contenu}
                </span>
              </div>
              <button type="button" onClick={() => setRepondA(null)} title="Annuler la réponse" aria-label="Annuler la réponse"
                style={{ background: "none", border: "none", padding: 4, flexShrink: 0, display: "flex" }}>
                <X size={13} color={C.mutedSoft} />
              </button>
            </div>
          )}

          <form onSubmit={envoyer} style={{ padding: "10px 14px 12px", borderTop: repondA ? "none" : `1px solid ${C.borderSoft}` }}>
            <div className="flex items-end gap-2 zone-saisie" style={{ border: `1px solid ${C.border}`, borderRadius: 14, padding: 5, background: C.surface }}>
              <div ref={paletteRef} style={{ position: "relative", flexShrink: 0 }}>
                <button type="button" onClick={() => setEmojisOuverts((v) => !v)} title="Insérer un émoji" aria-label="Insérer un émoji" aria-expanded={emojisOuverts}
                  style={{ background: emojisOuverts ? C.canvas : "transparent", border: "none", borderRadius: 10, padding: 8, display: "flex" }}>
                  <Smile size={18} color={emojisOuverts ? C.ink : C.mutedSoft} />
                </button>
                {emojisOuverts && (
                  <div className="apparait-haut" style={{ position: "absolute", bottom: "calc(100% + 10px)", left: 0, width: 276, background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: 10, boxShadow: "0 14px 34px rgba(0,11,83,0.16)", zIndex: 20 }}>
                    {EMOJIS.map((g) => (
                      <div key={g.groupe} style={{ marginBottom: 6 }}>
                        <div style={{ fontSize: 10, color: C.mutedSoft, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 3 }}>{g.groupe}</div>
                        <div className="flex flex-wrap" style={{ gap: 1 }}>
                          {g.liste.map((e) => (
                            <button key={e} type="button" onClick={() => insererEmoji(e)} title={e} className="emoji-choix"
                              style={{ background: "none", border: "none", borderRadius: 7, padding: "3px 4px", fontSize: 18, lineHeight: 1.2, cursor: "pointer" }}>
                              {e}
                            </button>
                          ))}
                        </div>
              ...
              <textarea ref={saisieRef} value={texte} rows={1} maxLength={2000}
                onChange={(ev) => setTexte(ev.target.value)}
                onKeyDown={(ev) => {
                  if (ev.key === "Enter" && !ev.shiftKey && !ev.nativeEvent.isComposing) { ev.preventDefault(); envoyer(ev); }
                }}
                placeholder={canal.type === "direct" ? `Message à ${titreCanal}…` : `Message dans ${titreCanal}…`}
                aria-label="Votre message"
                style={{ flex: 1, minWidth: 0, border: "none", outline: "none", resize: "none", background: "transparent", padding: "8px 4px", fontSize: 13, lineHeight: 1.45, maxHeight: 120 }} />
              <button type="submit" disabled={envoi || !texte.trim()} title="Envoyer (Entrée)"
                style={{ background: texte.trim() ? C.sun : C.canvas, color: texte.trim() ? C.ink : C.mutedSoft, border: "none", borderRadius: 10, padding: "8px 14px", fontSize: 12.5, fontWeight: 700, display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
                {envoi ? <Loader2 size={13} className="animate-spin" /> : <ArrowRight size={14} />} Envoyer
              </button>
            </div>
            <div style={{ fontSize: 10.5, color: C.mutedSoft, marginTop: 5, paddingLeft: 4 }}>
                ...
                {!showNewCampagne ? (
                  ...
                  <>
                    ...
                    <button onClick={createCampagne} disabled={creatingCampagne}
                      style={{ background: C.sun, color: C.ink, border: "none", borderRadius: 7, padding: "8px 12px", fontSize: 12, fontWeight: 700, display: "flex", alignItems: "center", gap: 6 }}>
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={createLot} disabled={creatingLot || !newLotNom.trim() || !newLotCibleId}
                    style={{ flex: 1, background: C.sun, color: C.ink, border: "none", borderRadius: 7, padding: "8px 0", fontSize: 12.5, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
            ...
            <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: 16, marginTop: 16, maxWidth: 640 }}>
              ...
              <div style={{ border: `1px solid ${C.borderSoft}`, borderRadius: 9, overflow: "hidden" }}>
                {champs.map((ch, i) => (
                  <div key={ch.cle} className="flex items-center gap-3"
                    ...
                    <input type="checkbox" checked={ch.coche} style={{ flexShrink: 0, width: 15, height: 15, accentColor: C.ink }}
              ...
              <div className="flex items-center gap-2" style={{ marginTop: 14 }}>
                <button onClick={() => finalizeImport(apercu.rows, apercu.fileName, apercu.parCle, champs)} disabled={busy}
                  style={{ background: C.sun, color: C.ink, border: "none", borderRadius: 8, padding: "10px 16px", fontSize: 12.5, fontWeight: 700, display: "flex", alignItems: "center", gap: 7 }}>
          ...
          {campagnes.map((c) => {
            ...
            return (
              <button key={c.id} onClick={() => { setSelected(c.id); setEditingNom(false); setShowAttach(false); }}
                ...
                <div style={{ fontSize: 11, color: isActive ? "#A9AED6" : C.muted, marginTop: 2 }}>
                ...
                <div style={{ background: C.canvas, borderRadius: 8, padding: 12, marginBottom: 12 }}>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => attachLot(current.id)} disabled={attaching || !attachNom.trim() || !attachCibleId}
                      style={{ background: C.sun, color: C.ink, border: "none", borderRadius: 7, padding: "7px 14px", fontSize: 12, fontWeight: 700, display: "flex", alignItems: "center", gap: 6, whiteSpace: "nowrap" }}>
                              ...
                              <>
                                <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, overflow: "hidden" }}>
                                  {editChamps.map((ch, i) => (
                                    <div key={ch.cle} className="flex items-center gap-2"
                                      ...
                                      <input type="checkbox" checked={ch.coche} style={{ flexShrink: 0, width: 14, height: 14, accentColor: C.ink }}
                                ...
                                <div className="flex items-center gap-2" style={{ marginTop: 8 }}>
                                  <button onClick={() => enregistrerChamps(l.id)} disabled={savingChamps}
                                    style={{ background: C.sun, color: C.ink, border: "none", borderRadius: 6, padding: "6px 12px", fontSize: 11.5, fontWeight: 700, display: "flex", alignItems: "center", gap: 5 }}>
      ...
      <form onSubmit={createCompte} style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: 18, marginBottom: 20, maxWidth: 620 }}>
        ...
        <div className="flex items-center gap-2">
          ...
          <button type="submit" disabled={creating}
            style={{ background: C.sun, color: C.ink, border: "none", borderRadius: 7, padding: "9px 16px", fontSize: 13, fontWeight: 700, display: "flex", alignItems: "center", gap: 6, whiteSpace: "nowrap" }}>
        ...
        <div style={{ background: C.surface, border: `1.5px solid ${C.ink}`, borderRadius: 12, padding: 18, marginBottom: 20, maxWidth: 640 }}>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={createCategory} disabled={creatingCat}
              style={{ background: C.sun, color: C.ink, border: "none", borderRadius: 7, padding: "8px 14px", fontSize: 12.5, fontWeight: 700, display: "flex", alignItems: "center", gap: 6, whiteSpace: "nowrap" }}>
          ...
          <div style={{ borderTop: `1px solid ${C.border}`, marginTop: 12, paddingTop: 12 }}>
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => onApparence(cat.categorie, { lien: lienSaisi.trim() || null })} disabled={!lienChange || lienRefuse}
                style={{ background: !lienChange || lienRefuse ? C.border : C.sun, color: C.ink, border: "none", borderRadius: 6, padding: "6px 11px", fontSize: 11.5, fontWeight: 700, flexShrink: 0 }}>
                Enregistrer
              </button>
            </div>
